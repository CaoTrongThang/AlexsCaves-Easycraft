...
public class JellyBeanItem extends PotionItem {
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack stack, int useDir) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            serverLevel.sendParticles(new ItemParticleOption(ACParticleRegistry.JELLY_BEAN_EAT.get(), stack), particlePos.x, particlePos.y, particlePos.z, 1, motion.x, motion.y + 0.05D, motion.z, 0.0D);
        } else {
            level.addParticle(new ItemParticleOption(ACParticleRegistry.JELLY_BEAN_EAT.get(), stack), particlePos.x, particlePos.y, particlePos.z, motion.x, motion.y + 0.05D, motion.z);
        }
    }

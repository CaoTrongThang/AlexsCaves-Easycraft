...
public class DreadbowItem extends ProjectileWeaponItem implements UpdatesStackTags {
    ...
    public static EntityType<?> getTypeOfArrow(ItemStack stack) {
        ...
        if (tag.contains("LastUsedArrowType")) {
            String typeId = tag.getString("LastUsedArrowType");
            return BuiltInRegistries.ENTITY_TYPE.getValue(ResourceLocation.parse(typeId));
        }
        return null;
    }
    ...
    public static float getLerpedUseTime(ItemStack stack, float partialTicks) {
        CompoundTag tag = getCustomData(stack);
        float prev = tag.getInt("PrevUseTime");
        float current = tag.getInt("UseTime");
            ...
            if (power > 0.1D) {
                ...
                if (abstractArrow != null) {
                    ...
                    for (int j = 0; j < Math.ceil(maxArrows * power); j++) {
                        ...
                        Vec3 spawnPos = mutableSkyPos.getCenter().add(level.random.nextFloat() * 16 - 8, level.random.nextFloat() * 4 - 2, level.random.nextFloat() * 16 - 8);
                        int clearTries = 0;
                        while (clearTries < 6 && !level.isEmptyBlock(BlockPos.containing(spawnPos)) && level.getFluidState(BlockPos.containing(spawnPos)).isEmpty()) {
                            clearTries++;
                            spawnPos = mutableSkyPos.getCenter().add(level.random.nextFloat() * 16 - 8, level.random.nextFloat() * 4 - 2, level.random.nextFloat() * 16 - 8);
                        }
                        ...
                        abstractArrow.shoot(shot.x, shot.y, shot.z, 0.5F + 1.5F * level.random.nextFloat(), randomness);
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack itemStack, int timeUsing) {
        ...
        if (living instanceof Player player && ACEnchantmentHelper.hasEnchantment(level, ACEnchantmentRegistry.RELENTLESS_DARKNESS, itemStack) && timeUsing % 3 == 0) {
            ...
            for (int i = 0; i < maxArrows; i++) {
                ...
                abstractArrow.shoot(shot.x, shot.y, shot.z, 4F + 3F * level.random.nextFloat(), randomness);
                randomness += 2.0F;
                level.addFreshEntity(abstractArrow);
                abstractArrow = createArrow(player, itemStack, ammoStack);
                if (abstractArrow != null) {

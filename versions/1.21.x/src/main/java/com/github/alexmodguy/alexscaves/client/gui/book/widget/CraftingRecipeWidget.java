...
public class CraftingRecipeWidget extends BookWidget {
    ...
    public void render(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, float partialTicks, boolean onFlippingPage) {
        ...
        if (smelting) {
            Ingredient ingredient = recipe.placementInfo().ingredients().isEmpty() ? Ingredient.of() : recipe.placementInfo().ingredients().get(0);
            ItemStack ingredientStack = pickStack(ingredient, playerTicks);
    ...
    private NonNullList<Ingredient> getDisplayIngredients(Recipe<?> recipe) {
        ...
        if (recipe instanceof ShapedRecipe shapedRecipe) {
            for (Optional<Ingredient> ingredient : shapedRecipe.getIngredients()) {
                ingredients.add(ingredient.orElseGet(Ingredient::of));
            }
            return ingredients;
        }
    ...
    private ItemStack getRecipeResult(Recipe<?> recipe, NonNullList<ItemStack> displayedStacks, int width, int height) {
        ...
        try {
            ...
            if (recipe instanceof AbstractCookingRecipe) {
                ...
                return ((Recipe<SingleRecipeInput>) recipe).assemble(new SingleRecipeInput(ingredient), Minecraft.getInstance().level.registryAccess());
            }
            ...
            return ((Recipe<CraftingInput>) recipe).assemble(CraftingInput.of(width, height, craftingStacks), Minecraft.getInstance().level.registryAccess());
        } catch (Exception e) {
            return ItemStack.EMPTY;
        }
    }
    ...
    private Recipe<?> getRecipeByName(String registryName) {
        try {
            if (Minecraft.getInstance().getConnection() != null && Minecraft.getInstance().getConnection().recipes() instanceof RecipeManager manager) {
                ResourceKey<net.minecraft.world.item.crafting.Recipe<?>> key = ResourceKey.create(Registries.RECIPE, ResourceLocation.parse(registryName));

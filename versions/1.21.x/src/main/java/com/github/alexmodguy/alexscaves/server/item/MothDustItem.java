    ...
    public boolean releaseUsing(ItemStack itemStack, Level level, LivingEntity living, int useTimeLeft) {
        if (living instanceof Player player) {
            ...
            for (int j = 0; j < Math.ceil(distance * 3); j++) {
                Vec3 spreadPos = hitPos.add(level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F);
                Vec3 startPos = player.getEyePosition().add(level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F);

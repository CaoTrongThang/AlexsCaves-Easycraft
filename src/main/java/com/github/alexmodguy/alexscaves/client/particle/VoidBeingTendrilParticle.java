...
public class VoidBeingTendrilParticle extends Particle {
        ...
        private static final ResourceLocation TENDRIL_TEXTURE = new ResourceLocation(AlexsCaves.MODID,
                        "textures/particle/void_being_cloud_tendril.png");
        ...
        private VoidBeingTendrilParticle(ClientLevel world, double x, double y, double z, double targetId,
                        double seekByTime) {
                super(world, x, y, z);
                this.setSize(1, 1);
                this.gravity = 0.0F;
                ...
                this.lifetime = 300;
                this.size = 1.0F;
                tipTarget = new Vec3(random.nextFloat() - 0.5F, random.nextFloat() - 0.5F, random.nextFloat() - 0.5F)
                                .normalize().scale(12).add(x, y, z);
        ...
        public void tick() {
                ...
                if (tipTarget != null) {
                        ...
                        if (entityTarget == null) {
                                tippening = tippening.add((float) Math.sin(ageSmooth + animationOffset),
                                                (float) Math.cos(ageSmooth - Math.PI / 2 + animationOffset),
                                                -(float) Math.cos(ageSmooth + animationOffset));
                        }
                        ...
                        if (entityTarget == null && random.nextFloat() < 0.1F) {
                                tipTarget = new Vec3(random.nextFloat() - 0.5F, random.nextFloat() - 0.5F,
        ...
        public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
                ...
                VertexConsumer vertexconsumer = multibuffersource$buffersource
                                .getBuffer(ACRenderTypes.itemEntityTranslucentCull(TENDRIL_TEXTURE));
                PoseStack posestack = new PoseStack();
                posestack.pushPose();
                posestack.translate(-cameraPos.x + x + cameraOffset.x,
                                -cameraPos.y + y + cameraOffset.y
                                                + 0.2F * (float) Math.sin((age + partialTick + animationOffset) * 0.1F),
                                -cameraPos.z + z + cameraOffset.z);
                int samples = 1;
                ...
                while (samples <= sampleCount) {
                        ...
                        Vec3 drawTo = getTendrilPosition(samplesScale, wiggleAmount, new Vec3(cameraOffset),
                                        partialTick);
                        ...
                        vertexconsumer
                                        .vertex(matrix4f, (float) drawFrom.x + (float) bottomAngleVec.x,
                                                        (float) drawFrom.y + (float) bottomAngleVec.y,
                                                        (float) drawFrom.z + (float) bottomAngleVec.z)
                                        .color(1F, 1F, 1F, startA).uv(u1, 1F).overlayCoords(NO_OVERLAY).uv2(j)
                                        .normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                        vertexconsumer
                                        .vertex(matrix4f, (float) drawTo.x + (float) bottomAngleVec.x,
                                                        (float) drawTo.y + (float) bottomAngleVec.y,
                                                        (float) drawTo.z + (float) bottomAngleVec.z)
                                        .color(1F, 1F, 1F, endA).uv(u2, 1F).overlayCoords(NO_OVERLAY).uv2(j)
                                        .normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                        vertexconsumer
                                        .vertex(matrix4f, (float) drawTo.x + (float) topAngleVec.x,
                                                        (float) drawTo.y + (float) topAngleVec.y,
                                                        (float) drawTo.z + (float) topAngleVec.z)
                                        .color(1F, 1F, 1F, endA).uv(u2, 0).overlayCoords(NO_OVERLAY).uv2(j)
                                        .normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                        vertexconsumer
                                        .vertex(matrix4f, (float) drawFrom.x + (float) topAngleVec.x,
                                                        (float) drawFrom.y + (float) topAngleVec.y,
                                                        (float) drawFrom.z + (float) topAngleVec.z)
                                        .color(1F, 1F, 1F, startA).uv(u1, 0).overlayCoords(NO_OVERLAY).uv2(j)
                                        .normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                        samples++;
                        drawFrom = drawTo;
                }
        ...
        private Vec3 getTendrilPosition(float f, float wiggleAmount, Vec3 cameraOffset, float partialTick) {
                ...
                Vec3 wiggleVec = new Vec3((float) Math.sin(ageSmooth + f + animationOffset),
                                (float) Math.cos(ageSmooth - Math.PI / 2 + f + animationOffset),
                                -(float) Math.cos(ageSmooth + f + animationOffset)).scale(wiggleAmount * 0.2F);
        ...
        private Quaternionf calcCameraAngle(Camera camera, float sampleScale) {
                ...
                return new Quaternionf(camera.rotation()).rotateX(followCameraAmount * -0.017453292F * camera.getXRot())
                                .rotateY(followCameraAmount * 0.017453292F * camera.getYRot());
        }

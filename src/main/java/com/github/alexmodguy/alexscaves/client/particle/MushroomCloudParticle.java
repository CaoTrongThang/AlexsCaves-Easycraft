...
public class MushroomCloudParticle extends Particle {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(AlexsCaves.MODID,
            "textures/particle/mushroom_cloud.png");
    private static final ResourceLocation TEXTURE_GLOW = new ResourceLocation(AlexsCaves.MODID,
            "textures/particle/mushroom_cloud_glow.png");
    private static final ResourceLocation TEXTURE_PINK = new ResourceLocation(AlexsCaves.MODID,
            "textures/particle/mushroom_cloud_pink.png");
    private static final ResourceLocation TEXTURE_PINK_GLOW = new ResourceLocation(AlexsCaves.MODID,
            "textures/particle/mushroom_cloud_pink_glow.png");
        ...
        if (age > BALL_FOR / 2 + 5) {
            if (!playedExplosion) {
                ...
                playSound(
                        ...
                        lifetime - 20, lifetime, 0.2F, false);
            }
        }
        if (age < BALL_FOR) {
            ...
            for (int i = 0; i < (1 + random.nextInt(2)) * scale; i++) {
                Vec3 from = new Vec3(level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F,
                        level.random.nextFloat() - 0.5F).scale(scale * 1.4F).add(this.x, this.y, this.z);
                Vec3 away = new Vec3(level.random.nextFloat() - 0.5F, level.random.nextFloat() - 0.5F,
                        level.random.nextFloat() - 0.5F).scale(2.34F);
                this.level.addParticle(ACParticleRegistry.MUSHROOM_CLOUD_SMOKE.get(), from.x, from.y, from.z, away.x,
                        away.y, away.z);
            }
            for (int j = 0; j < scale * scale; j++) {
                Vec3 explosionBase = new Vec3((level.random.nextFloat() - 0.5F) * explosionSpread,
                        (-0.6F + level.random.nextFloat() * 0.5F) * explosionSpread * 0.1F,
                        (level.random.nextFloat() - 0.5F) * explosionSpread).add(this.x, this.y, this.z);
                this.level.addParticle(ACParticleRegistry.MUSHROOM_CLOUD_EXPLOSION.get(), explosionBase.x,
                        explosionBase.y, explosionBase.z, 0, 0, 0);
            }
            if (age > BALL_FOR) {
                if (!playedRumble) {
                    playedRumble = true;
    ...
    private void playSound(SoundEvent soundEvent, int duration, int fadesAt, float fadeInBy, boolean looping) {
        Minecraft.getInstance().getSoundManager().queueTickingSound(
                new NuclearExplosionSound(soundEvent, this.x, this.y, this.z, duration, fadesAt, fadeInBy, looping));
    }
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        VertexConsumer baseConsumer = multibuffersource$buffersource
                .getBuffer(RenderType.entityTranslucent(pink ? TEXTURE_PINK : TEXTURE));
        MODEL.renderToBuffer(posestack, baseConsumer, getLightColor(partialTick), OverlayTexture.NO_OVERLAY, 1.0F, 1.0F,
                1.0F, alpha);
        VertexConsumer glowConsumer1 = multibuffersource$buffersource
                .getBuffer(ACRenderTypes.getEyesAlphaEnabled(pink ? TEXTURE_PINK : TEXTURE));
        MODEL.renderToBuffer(posestack, glowConsumer1, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, alpha);
        VertexConsumer glowConsumer2 = multibuffersource$buffersource
                .getBuffer(ACRenderTypes.getEyesAlphaEnabled(pink ? TEXTURE_PINK_GLOW : TEXTURE_GLOW));
        MODEL.renderToBuffer(posestack, glowConsumer2, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F,
                glowLife * alpha);
        posestack.popPose();
    }

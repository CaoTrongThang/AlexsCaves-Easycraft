...
public class HullbreakerInspectMobGoal extends Goal {
        ...
        if (!list.isEmpty()) {
            ...
            for (LivingEntity mob : list) {
                if ((closest == null || mob.distanceToSqr(entity) < closest.distanceToSqr(entity))
                        && entity.hasLineOfSight(mob) && !mob.is(entity)) {
                    closest = mob;
                }
            }
        ...
        if (entity.getRandom().nextInt(20) == 0 && !HullbreakerEntity.GLOWING_TARGET.test(inspectingTarget)) {
            ...
            if (staring) {
                if (isPreyWatching() && distance < 18) {
                    ...
                    Vec3 frontVision = inspectingTarget.getEyePosition()
                ...
                Vec3 circle = orbitAroundPos(inspectingTarget.getEyePosition(),
                        12 + Math.min(0, 5 - entity.getInterestLevel()) * 3);
                entity.getNavigation().moveTo(circle.x, circle.y, circle.z, 1.4F);
                entity.setYHeadRot(
                        Mth.approachDegrees(entity.getYHeadRot(), entity.yBodyRot + (clockwise ? 30 : -30), 10));
    ...
    public boolean isPreyWatching() {
        ...
        if (vec31.distanceTo(vec3) > 128.0D) {
            ...
            return entity.level()
                    .clip(new ClipContext(vec3, vec31, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, lowestPrey))

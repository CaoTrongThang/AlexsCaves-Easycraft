...
public class ACSurfaceRules {
    ...
    public static SurfaceRules.RuleSource createMagneticCavesRules() {
        ...
        SurfaceRules.ConditionSource azureCondition = ACSurfaceRuleConditionRegistry.simplexCondition(-0.025F, 0.025F,
                90, 1F, 0);
        SurfaceRules.ConditionSource scarletCondition = ACSurfaceRuleConditionRegistry.simplexCondition(-0.025F, 0.025F,
                90, 1F, 1);
        return SurfaceRules.sequence(bedrock(),
                SurfaceRules.ifTrue(azureCondition, SurfaceRules.ifTrue(scarletCondition, neutral)),
                SurfaceRules.ifTrue(scarletCondition, scarlet), SurfaceRules.ifTrue(azureCondition, azure), galena);
    }
    ...
    public static SurfaceRules.RuleSource createPrimordialCavesRules() {
        ...
        SurfaceRules.RuleSource dirtOrPackedMud = SurfaceRules.sequence(
                SurfaceRules.ifTrue(SurfaceRules.noiseCondition(Noises.GRAVEL, -0.12D, 0.2D), packedMud), dirt);
        SurfaceRules.ConditionSource isUnderwater = SurfaceRules.waterBlockCheck(0, 0);
        SurfaceRules.RuleSource grassWaterChecked = SurfaceRules.sequence(SurfaceRules.ifTrue(isUnderwater, grass),
                dirtOrPackedMud);
    ...
    public static SurfaceRules.RuleSource createAbyssalChasmRules() {
        ...
        return SurfaceRules.sequence(bedrock(), SurfaceRules.ifTrue(SurfaceRules.DEEP_UNDER_FLOOR, stoneOrDeepslate),
    ...
    public static SurfaceRules.RuleSource createForlornHollowsRules() {
        ...
        SurfaceRules.ConditionSource corpolithCondition = ACSurfaceRuleConditionRegistry.simplexCondition(-0.2F, 0.4F,
                40, 6F, 3);
    ...
    public static SurfaceRules.RuleSource createCandyCavityRules() {
        ...
        return SurfaceRules.sequence(bedrock(), floorRules,
                createBands(20, 2, 10, ACBlockRegistry.BLOCK_OF_CHOCOLATE.get().defaultBlockState()), cake);
    }
    ...
    private static SurfaceRules.RuleSource createBands(int layers, int layerThickness, int layerDistance,
        ...
        for (int i = 1; i <= layers; i++) {
            ...
            ruleSources[i - 1] = SurfaceRules.ifTrue(layer1, SurfaceRules.ifTrue(SurfaceRules.not(layer2),
                    SurfaceRules.ifTrue(SurfaceRules.noiseCondition(Noises.ICE, -0.7D, 0.8D), sandstone)));
        }

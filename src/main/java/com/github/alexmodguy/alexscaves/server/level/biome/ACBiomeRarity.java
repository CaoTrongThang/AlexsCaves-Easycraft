...
public class ACBiomeRarity {
    ...
    private static final PerlinSimplexNoise NOISE_X = new PerlinSimplexNoise(new XoroshiroRandomSource(1234L),
            BIOME_OCTAVES);
    private static final PerlinSimplexNoise NOISE_Z = new PerlinSimplexNoise(new XoroshiroRandomSource(4321L),
            BIOME_OCTAVES);
    ...
    /**
     * Does the heavy lifting of finding if x and z quads should contain a rare
     * biome.
     *
    ...
    public static VoronoiGenerator.VoronoiInfo getRareBiomeInfoForQuad(long worldSeed, int x, int z) {
        ...
        double positionOffsetX = AlexsCaves.COMMON_CONFIG.caveBiomeWidthRandomness.get()
                * NOISE_X.getValue(sampleX, sampleZ, false);
        double positionOffsetZ = AlexsCaves.COMMON_CONFIG.caveBiomeWidthRandomness.get()
                * NOISE_Z.getValue(sampleX, sampleZ, false);
        VoronoiGenerator.VoronoiInfo info = VORONOI_GENERATOR.get2(sampleX + positionOffsetX,
                sampleZ + positionOffsetZ);
    ...
    /**
     * gets the center of the biome pertaining to the voronoiInfo. Result is in quad
     * coordinates.
     *
    ...
    /**
     * gets the 'rarityOffset' for voroniInfo, which is essentially an internal
     * biome ID.
     *
     * @param voronoiInfo the info of the sampled biome

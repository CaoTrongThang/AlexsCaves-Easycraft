...
import com.github.alexmodguy.alexscaves.forge_shim.event.AnvilUpdateEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.TickEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.EntityEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.EntityTravelToDimensionEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.living.*;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.player.AttackEntityEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.player.PlayerEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.entity.player.PlayerInteractEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.server.ServerAboutToStartEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.server.ServerStoppingEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.village.VillagerTradesEvent;
import com.github.alexmodguy.alexscaves.forge_shim.event.village.WandererTradesEvent;
import com.github.alexmodguy.alexscaves.forge_shim.eventbus.api.EventPriority;
...
public class CommonEvents {
    ...
    public void resizeEntity(EntityEvent.Size event) {
        if (event.getEntity().isAlive() && event.getEntity() instanceof MagneticEntityAccessor magnet
                && event.getEntity().getEntityData().isDirty()) {
            ...
            if (dir == Direction.DOWN && event.getEntity() instanceof Player
                    && event.getEntity().getPose() == Pose.STANDING) {
    ...
    public void livingDie(LivingDeathEvent event) {
        if (event.getEntity().getType() == EntityType.MAGMA_CUBE && event.getSource() != null
                && event.getSource().getEntity() instanceof Frog frog) {
        ...
        if (!event.getEntity().level().isClientSide && event.getEntity() instanceof Mob mob && event.getSource() != null
                && event.getSource().getDirectEntity() instanceof LivingEntity directSource
                && directSource.getItemInHand(InteractionHand.MAIN_HAND).is(ACItemRegistry.PRIMITIVE_CLUB.get())) {
            if (com.github.alexmodguy.alexscaves.fabric.ItemStackCompat.getEnchantmentLevel(
                    directSource.getItemInHand(InteractionHand.MAIN_HAND), ACEnchantmentRegistry.BONKING.get()) > 0
                    && event.getEntity().level().random.nextFloat() < 0.33F) {
        ...
        if (event.getEntity() instanceof Player) {
            if (event.getEntity().getUUID().toString().equals("71363abe-fd03-49c9-940d-aae8b8209b7c")) {
                event.getEntity().spawnAtLocation(new ItemStack(ACItemRegistry.GREEN_SOYLENT.get(),
                        1 + event.getEntity().getRandom().nextInt(9)));
            }
    ...
    public void livingHeal(LivingHealEvent event) {
        if (event.getEntity().hasEffect(ACEffectRegistry.IRRADIATED.get())
                && !event.getEntity().getType().is(ACTagRegistry.RESISTS_RADIATION)) {
    ...
    public void playerEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (stack.is(ACItemRegistry.HOLOCODER.get()) && event.getTarget() instanceof LivingEntity
                && !(event.getTarget() instanceof ArmorStand) && event.getTarget().isAlive()) {
            CompoundTag tag = stack.getOrCreateTag();
            tag.putUUID("BoundEntityUUID", event.getTarget().getUUID());
            CompoundTag entityTag = event.getTarget() instanceof Player ? new CompoundTag()
                    : event.getTarget().saveWithoutId(new CompoundTag());
    ...
    public void livingFindTarget(LivingChangeTargetEvent event) {
        if (event.getEntity() instanceof Mob mob && event.getNewTarget() instanceof VallumraptorEntity vallumraptor
                && vallumraptor.getHideFor() > 0) {
            mob.setTarget(null);
            event.setCanceled(true);
        }
    }
    ...
    public void livingHurt(LivingDamageEvent event) {
        if (event.getEntity().isPassenger() && event.getEntity() instanceof FlyingMount
                && (event.getSource().is(DamageTypes.IN_WALL) || event.getSource().is(DamageTypes.FALL)
                        || event.getSource().is(DamageTypes.FLY_INTO_WALL))) {
            event.setCanceled(true);
        }
        if (event.getEntity() instanceof WatcherPossessionAccessor possessed && possessed.isPossessedByWatcher()
                && !event.getSource().is(DamageTypeTags.BYPASSES_INVULNERABILITY)
                && !(event.getSource().getEntity() instanceof WatcherEntity)) {
            event.setCanceled(true);
        }
        if (event.getEntity() instanceof Player player && player.getUseItem().is(ACItemRegistry.EXTINCTION_SPEAR.get())
        ...
        if (event.getEntity() instanceof Player player && event.getSource().is(DamageTypes.FALL)
                && player.getItemBySlot(EquipmentSlot.FEET).is(ACItemRegistry.RAINBOUNCE_BOOTS.get())) {
    ...
    public void livingAttack(LivingAttackEvent event) {
        if (event.getSource().getDirectEntity() instanceof AbstractArrow arrow && event.getEntity().isBlocking()
                && event.getEntity().getUseItem().is(ACItemRegistry.RESISTOR_SHIELD.get())) {
            ItemStack shield = event.getEntity().getUseItem();
            if (com.github.alexmodguy.alexscaves.fabric.ItemStackCompat.getEnchantmentLevel(shield,
                    ACEnchantmentRegistry.ARROW_INDUCTING.get()) > 0
                    && arrow.getType() != ACEntityRegistry.SEEKING_ARROW.get()) {
                SeekingArrowEntity seekingArrowEntity = new SeekingArrowEntity(event.getEntity().level(),
                        event.getEntity());
        ...
        if (event.getSource() != null && event.getSource().getDirectEntity() instanceof LivingEntity directSource
                && directSource.hasEffect(ACEffectRegistry.STUNNED.get())) {
            event.setCanceled(true);
        }
    }
    ...
    public void livingTick(LivingEvent.LivingTickEvent event) {
        if (event.getEntity().hasEffect(ACEffectRegistry.BUBBLED.get())
                && com.github.alexmodguy.alexscaves.fabric.EntityCompat.isInFluidType(event.getEntity())) {
            event.getEntity().removeEffect(ACEffectRegistry.BUBBLED.get());
        }
        if (event.getEntity().hasEffect(ACEffectRegistry.DARKNESS_INCARNATE.get())
                && event.getEntity().tickCount % 5 == 0 && DarknessIncarnateEffect.isInLight(event.getEntity(), 11)) {
            event.getEntity().removeEffect(ACEffectRegistry.DARKNESS_INCARNATE.get());
        }
        if (event.getEntity().getItemBySlot(EquipmentSlot.HEAD).is(ACItemRegistry.DIVING_HELMET.get())
                && (!event.getEntity().isEyeInFluid(FluidTags.WATER)
                        || event.getEntity().getVehicle() instanceof SubmarineEntity)) {
        ...
        if (!event.getEntity().level().isClientSide && event.getEntity() instanceof Mob mob
                && mob.getTarget() instanceof VallumraptorEntity vallumraptor && vallumraptor.getHideFor() > 0) {
            mob.setTarget(null);
        }
    }
        ...
        try {
            if (event.getEntity() instanceof Creeper creeper) {
                creeper.targetSelector.addGoal(3,
                        new AvoidEntityGoal<>(creeper, RaycatEntity.class, 10.0F, 1.0D, 1.2D));
            }
            if (event.getEntity() instanceof Drowned drowned
                    && drowned.level().getBiome(drowned.blockPosition()).is(ACBiomeRegistry.ABYSSAL_CHASM)) {
                if (drowned.getItemBySlot(EquipmentSlot.FEET).isEmpty()
                        && drowned.getItemBySlot(EquipmentSlot.LEGS).isEmpty()
                        && drowned.getItemBySlot(EquipmentSlot.CHEST).isEmpty()
                        && drowned.getItemBySlot(EquipmentSlot.HEAD).isEmpty()) {
            ...
            if (event.getEntity() instanceof Fox fox) {
                fox.targetSelector.addGoal(7,
                        new NearestAttackableTargetGoal<>(fox, GingerbreadManEntity.class, 40, false, false, null));
            }
        } catch (Exception e) {
    ...
    public void livingRemoveEffect(MobEffectEvent.Remove event) {
        if (event.getEffect() instanceof DarknessIncarnateEffect darknessIncarnateEffect) {
            ...
            event.getEntity().level().playSound(null, event.getEntity().getX(), event.getEntity().getY(),
                    event.getEntity().getZ(), ACSoundRegistry.DARKNESS_INCARNATE_EXIT.get(), SoundSource.PLAYERS, 1.0F,
                    1.0F);
        }
        if (event.getEffect() instanceof SugarRushEffect) {
            event.getEntity().level().playSound(null, event.getEntity().getX(), event.getEntity().getY(),
                    event.getEntity().getZ(), ACSoundRegistry.SUGAR_RUSH_EXIT.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
        }
    }
    ...
    public void livingAddEffect(MobEffectEvent.Added event) {
        if (event.getEffectInstance().getEffect() instanceof DarknessIncarnateEffect) {
            event.getEntity().level().playSound(null, event.getEntity().getX(), event.getEntity().getY(),
                    event.getEntity().getZ(), ACSoundRegistry.DARKNESS_INCARNATE_ENTER.get(), SoundSource.PLAYERS, 1.0F,
                    1.0F);
        }
        if (event.getEffectInstance().getEffect() instanceof SugarRushEffect) {
            event.getEntity().level().playSound(null, event.getEntity().getX(), event.getEntity().getY(),
                    event.getEntity().getZ(), ACSoundRegistry.SUGAR_RUSH_ENTER.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
        }
        if (event.getEntity() instanceof Player player && player.isAlive()
                && event.getEffectInstance().getEffect() instanceof SugarRushEffect
                && AlexsCaves.COMMON_CONFIG.sugarRushSlowsTime.get()) {
            float timeBetweenTicksIncrease = 2F;
            SugarRushEffect.enterSlowMotion(player, player.level(),
                    Mth.ceil(event.getEffectInstance().getDuration() * timeBetweenTicksIncrease),
                    timeBetweenTicksIncrease);
        }
    }
    ...
    public void onServerAboutToStart(ServerAboutToStartEvent event) {
        ...
        for (ResourceKey<LevelStem> levelStemResourceKey : levelStems.registryKeySet()) {
            ...
            if (holderOptional.isPresent() && holderOptional.get().value().generator()
                    .getBiomeSource() instanceof BiomeSourceAccessor expandedBiomeSource) {
    ...
    public void onVillagerTradeSetup(VillagerTradesEvent event) {
        if (event.getType() == VillagerProfession.CARTOGRAPHER
                && AlexsCaves.COMMON_CONFIG.cartographersSellCabinMaps.get()) {
            int level = 2;
    ...
    public void playerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (AlexsCaves.COMMON_CONFIG.warnGenerationIncompatibility.get()
                && !AlexsCaves.MOD_GENERATION_CONFLICTS.isEmpty() && event.getEntity().level().isClientSide) {
            for (String modid : AlexsCaves.MOD_GENERATION_CONFLICTS) {
                if (ModList.get().isLoaded(modid)) {
                    event.getEntity().sendSystemMessage(
                            Component.translatable("alexscaves.startup_warning.generation_incompatible", modid)
    ...
    private static void checkAndDestroyExploitItem(Player player, EquipmentSlot slot) {
        ...
        if (itemInHand.is(ACTagRegistry.RESTRICTED_BIOME_LOCATORS)) {
            ...
            if (tag != null) {
                if (itemTagContainsAC(tag, "BiomeKey", false) || itemTagContainsAC(tag, "Structure", true)
                        || itemTagContainsAC(tag, "structurecompass:structureName", true)
                        || itemTagContainsAC(tag, "StructureKey", true)) {
                    itemInHand.shrink(1);
                    player.broadcastBreakEvent(slot);
                    player.playSound(ACSoundRegistry.DISAPPOINTMENT.get());
                    if (!player.level().isClientSide) {
                        player.displayClientMessage(Component.translatable("item.alexscaves.natures_compass_warning"),
                                true);
                    }
                }
            }
        ...
        if (event.getItemStack().getItem() == Items.GLASS_BOTTLE) {
            ...
            if (raytraceresult.getType() == HitResult.Type.BLOCK) {
                ...
                if (event.getLevel().mayInteract(player, blockpos)) {
                    if (com.github.alexmodguy.alexscaves.fabric.FluidTypeCompat.getFluidType(
                            event.getLevel().getFluidState(blockpos)) == ACFluidRegistry.PURPLE_SODA_FLUID_TYPE.get()) {
    ...
    public void onUpdateAnvil(AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof AlwaysCombinableOnAnvil
                && event.getLeft().getItem() == event.getRight().getItem()
                && !EnchantmentHelper.getEnchantments(event.getLeft()).isEmpty()
                && !EnchantmentHelper.getEnchantments(event.getRight()).isEmpty()) {
    ...
    private static boolean itemTagContainsAC(CompoundTag tag, String tagID, boolean allowUndergroundCabin) {
        if (tag.contains(tagID)) {
            ...
            return resourceLocation.contains("alexscaves:")
                    && (!allowUndergroundCabin || !resourceLocation.contains("underground_cabin"));
        }
        return false;
    }
}

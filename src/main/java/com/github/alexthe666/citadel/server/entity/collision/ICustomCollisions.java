...
public interface ICustomCollisions {
    ...
    static Vec3 getAllowedMovementForEntity(Entity entity, Vec3 vecIN) {
        ...
        if (com.github.alexmodguy.alexscaves.fabric.EntityCompat.getStepHeight(entity) > 0.0F && flag3 && (flag || flag2)) {
            Vec3 vec31 = collideBoundingBox2(entity, new Vec3(vecIN.x, com.github.alexmodguy.alexscaves.fabric.EntityCompat.getStepHeight(entity), vecIN.z), aabb, entity.level(), list);
            Vec3 vec32 = collideBoundingBox2(entity, new Vec3(0.0D, com.github.alexmodguy.alexscaves.fabric.EntityCompat.getStepHeight(entity), 0.0D), aabb.expandTowards(vecIN.x, 0.0D, vecIN.z), entity.level(), list);
            if (vec32.y < (double) com.github.alexmodguy.alexscaves.fabric.EntityCompat.getStepHeight(entity)) {

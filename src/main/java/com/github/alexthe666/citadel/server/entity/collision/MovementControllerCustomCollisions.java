    ...
    public void tick() {
        if (this.operation == MoveControl.Operation.STRAFE) {
            ...
            if (!(mob instanceof ICustomCollisions && ((ICustomCollisions) mob).canPassThrough(blockpos, blockstate, voxelshape))) {
                if (d2 > (double) com.github.alexmodguy.alexscaves.fabric.EntityCompat.getStepHeight(this.mob) && d0 * d0 + d1 * d1 < (double) Math.max(1.0F, this.mob.getBbWidth()) || !voxelshape.isEmpty() && this.mob.getY() < voxelshape.max(Direction.Axis.Y) + (double) blockpos.getY() && !blockstate.is(BlockTags.DOORS) && !blockstate.is(BlockTags.FENCES)) {
